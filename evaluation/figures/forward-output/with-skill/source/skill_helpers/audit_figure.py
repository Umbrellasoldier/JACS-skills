...
def audit_pdf(path: Path, width_pt: float, height_pt: float, min_font_pt: float = 4.5) -> dict:
    ...
    for page in reader.pages:
        ...
        def visit(text, cm, tm, font, font_size):
            ...
            effective = abs(float(font_size)) * min(math.hypot(a, b), math.hypot(c, d))
            if effective > 0:
    ...
    return {
        ...
        "scope": "MediaBox and extractable transformed text; not all glyphs, paths, image DPI, chemical identity or data correctness",
    }
